...
public:
	...
	static const int SCORE_PER_ROW = 40;
...
private:
	...
	void drawBlock(const Point& topLeft, int xOffset, int yOffset, TetColor color, float alpha=1.0f);
	...
	void drawTetromino(const GridTetromino& tetromino, const Point& topLeft, float alpha=1.0f);
	...
	//shows current tetromino if the player drops their piece
    ...
    GridTetromino ghostShape;	// the tetromino that is currently falling.
